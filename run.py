    ...
    def golden_min(inp,xl,xu,frame4):
        ...
        while True:
            ...
            if ea<1:
                ...
                if x1bool==True:
                    Label(frame4,text='Max. y = '+str(maximum)).grid(row=7,column=1)
                    Label(frame4,text=' Max. x = '+str(x1)).grid(row=8,column=1)
                else:
                    Label(frame4,text='Max. y = '+str(maximum)).grid(row=7,column=1)
                    Label(frame4,text=' Max. x = '+str(x1)).grid(row=8,column=1)
                Label(frame4,text='Approximate Relative Error: '+str(ea)+'%').grid(row=9,column=1)
                break
            prev=maximum
            counter+=1
    ...
    def parab_interp(inp,x0,x1,x2,frame4):
        ...
        while True:
            ...
            if error<0.01:
                ...
                Label(frame4,text='Max. y = '+str(f.subs(x,x3).evalf())).grid(row=8,column=1)
                Label(frame4,text=' Max. x = '+str(x3)).grid(row=9,column=1)
                Label(frame4,text='Approximate Relative Error: '+str(error)+'%').grid(row=10,column=1)
                break
            if x3>x1:
                x0=x1
                x1=x3
    ...
    def newt_optim(inp,x0,frame4):
        ...
        while True:
            ...
            if ea<1:
                ...
                Label(frame4,text='Max. y = '+str(f.subs(x,x1).evalf())).grid(row=6,column=1)
                Label(frame4,text=' Max. x = '+str(x1)).grid(row=7,column=1)
                Label(frame4,text='Approximate Relative Error: '+str(ea)+'%').grid(row=8,column=1)
                break
            x0=x1
    def steep_ascent(inp,x0,y0,frame4):
        ...
        while True:
            ...
            if ea1<0.01 and ea2<0.01:
                ...
                Label(frame4,text='Approximate Relative Error: '+str((ea1+ea2)/2)+'%').grid(row=9,column=1)
                break
            prevx=x0
            prevy=y0
class curve_fitting:
    ...
    def nonlin_reg(x,y,inp,parameters,ig):
        ...
        if len(param)!=len(ig):
            messagebox.showerror('Error','Number of inital guesses must equal number of parameters')
            pass
        else:
            s=[]
            for i in param:
            ...
            while True:
                ...
                try:
                    LHS=np.linalg.inv(np.array(Ztran.dot(z),dtype='float64'))
                except BaseException:
                    Label(w4,text='').pack()
                    for i in range(len(a)):
                        Label(w4,text='a'+str(i)+' = '+str(a)).pack()
...
class integrate:
    ...
    def sing_onethird(inp,a,b,frame3,w3):
        ...
        for i in range(len(inp)-1):
            if inp[i+c].isnumeric() and inp[i+1+c].isalpha():
                ...
                c+=1
        f=parse_expr(inp)
        I=(b-a)*(f.subs(x,a).evalf()+4*f.subs(x,(b-a)/2)+f.subs(x,b).evalf())/6
        fp=sym.diff(f,x)
        fpp=sym.diff(fp,x)
        f3p=sym.diff(fpp,x)
        f4p=sym.diff(f3p,x)
    ...
    def threeEighth(inp,a,b,frame3,w3):
        ...
        f=parse_expr(inp)
        step=(b-a)/3
        I=(b-a)*(f.subs(x,a).evalf()+3*f.subs(x,a+step)+3*f.subs(x,a+step*2).evalf()+f.subs(x,b).evalf())/8
        fp=sym.diff(f,x)
        fpp=sym.diff(fp,x)
        f3p=sym.diff(fpp,x)
        f4p=sym.diff(f3p,x)
    ...
    def uneq_segments(x,fx,w3):
        ...
        Label(w3,text='').pack()
        Label(w3,text='Integral: '+str(I),font=("Times","12","bold")).pack()
class differentiate:
    def __init__(self):
        pass
    def fkutta_eval(df,end,h,x0,y0,frame2,w2):
        ...
        while x0<end:
            ...
            k3=(df.subs(x,x0+0.25*h).evalf()).subs(y,y0+0.125*k1*h+0.125*k2*h).evalf()
            k4=(df.subs(x,x0+0.5*h).evalf()).subs(y,y0-0.5*k2*h+k3*h).evalf()
            k5=(df.subs(x,x0+0.75*h).evalf()).subs(y,y0+(3/16)*k1*h+(9/16)*k4*h).evalf()
            k6=(df.subs(x,x0+h).evalf()).subs(y,y0-(3/7)*k1*h+(2/7)*k2*h+(12/7)*k3*h-(12/7)*k4*h+(8/7)*k5*h).evalf()
    ...
    def fif_rkutta(inp,end,h,x0,y0,frame2,w2):
        ...
        if Abs(x0-end)%h!=0:
            messagebox.showerror(title='Error',message="x-value to evaluate doesn't agree with step size value")
            w2.lift()
        else:
            x,y=sym.symbols('x y')
            #reformat the input
            ...
            Label(frame2,text='y('+str(end)+') = '+str(y1)).grid(row=7,column=1)
    def runge_kutta(inp,end,h,x0,y0,frame2,w2):
        ...
        if Abs(x0-end)%h!=0:
            ...
            while x0<end:
                k1=(df.subs(x,x0).evalf()).subs(y,y0).evalf()
                k2=(df.subs(x,x0+0.5*h).evalf()).subs(y,y0+0.5*k1*h).evalf()
                k3=(df.subs(x,x0+0.5*h).evalf()).subs(y,y0+0.5*k2*h).evalf()
    ...
    def midpoint(inp,end,h,x0,y0,frame2,w2):
        ...
        if Abs(x0-end)%h!=0:
            ...
            while x0<end:
                ...
                y1=y0+(df.subs(x,x0+(h/2)).evalf()).subs(y,y_half).evalf()
                
                tval=f.subs(x,i+h).evalf()
                tval=tval.subs(y,y1).evalf()
                et=100*Abs((tval-y1)/tval)
    ...
    def heun(inp,end,h,x0,y0,frame2,w2):
        ...
        if Abs(x0-end)%h!=0:
            ...
            while i<end:
                ...
                yip1=yi+((df.subs(x,x0).evalf()).subs(y,y0).evalf()+(df.subs(x,x0+h).evalf()).subs(y,y0ip1).evalf())*(h/2)
                if i+1==end:
                    break
                # 4e^(0.8*x)-0.5*y
                x0=x0+h
            ...
            iterations=1
            while et>1:
                yip1=yi+((df.subs(x,x0).evalf()).subs(y,y0).evalf()+(df.subs(x,x0+h).evalf()).subs(y,prev).evalf())*(h/2)
                ...
                iterations+=1
            Label(frame2,text='Solution').grid(row=6,column=1)
            Label(frame2,text='y('+str(end)+') = '+str(prev)).grid(row=7,column=1)
            Label(frame2,text='Error: '+str(et)+'%').grid(row=8,column=1)
            Label(frame2,text='Iterations: '+str(iterations)).grid(row=9,column=1)
    def euler(inp,end,h,x0,y0,frame2,w2):
        ...
        if Abs(x0-end)%h!=0:
            ...
            Label(frame2,text='Solution').grid(row=6,column=1)
            Label(frame2,text='y('+str(i)+') = '+str(y1)).grid(row=7,column=1)
            Label(frame2,text='True Relative Error: '+str(et)+'%').grid(row=8,column=1)           
...
class execute:
    ...
    def mod_newt(self,E1,E2,E3,frame):
        ...
        if 'log' in inp:
            ...
            x1=x0-((fx0*dfx0)/(dfx0*dfx0-fx0*df2x0))
            fx1=f.subs(x,x1).evalf()
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                Label(frame,text='Root:  '+str('{:.8}'.format(float(x1)))).grid(row=5,column=0,padx=10)
                if n<=6:
                    prev=Label(frame,text='Error: '+str("{:f}".format(float(error)))+'%')
                    prev.grid(row=6,column=0,padx=10)
                else:
                    prev=Label(frame,text='Error: '+str(float(error))+'%')
                    prev.grid(row=6,column=0,padx=10)
                
                Label(frame,text='Solution',font=("Times","12")).grid(row=4,column=0,padx=10)
    ...
    def newt(self,E1,E2,E3,frame):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                Label(frame,text='Root:  '+str(float('{:.8}'.format(x1)))).grid(row=5,column=0,padx=10)
                if n<=6:
                    prev=Label(frame,text='Error: '+str("{:f}".format(float(error)))+'%')
                    prev.grid(row=6,column=0,padx=10)
                else:
    ...
    def finish(self,E1,E2,E3,E4,frame):#secant method
        ...
        if 'log' in inp:
            ...
            tvalue=float(fsolve(f,xi_1))
            option=False

        error=(Abs(tvalue-xi)/Abs(tvalue))*100
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                Label(frame,text='Root:  '+str(float('{:.8}'.format(xi)))).grid(row=6,column=0,padx=10)
                if n<=6:
                    prev1=Label(frame,text='Error: '+str("{:f}".format(float(error)))+'%')
                    prev1.grid(row=7,column=0,padx=10)
                else:
                    prev1=Label(frame,text='Error: '+str(float(error))+'%')
                    prev1.grid(row=7,column=0,padx=10)
                
                Label(frame,text='Solution',font=("Times","12")).grid(row=5,column=0,padx=10)
            except RuntimeWarning:
                messagebox.showerror('Error','Solution Diverges')
                pass
            except TypeError:
    ...
    def mod_finish(self,E1,E2,E3,E4,frame):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                Label(frame,text='Root:  '+str(float('{:.8}'.format(x1)))).grid(row=6,column=0,padx=10)
                if n<=6:
                    prev2=Label(frame,text='Error: '+str("{:f}".format(float(error)))+'%')
