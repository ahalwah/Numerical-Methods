...
class read_in_constraint:
    ...
    def make_num(self, lists):
        for i in range(len(lists)):
            for x in range(len(lists[i])):
                print(lists[i][x])  # = float(lists[i][x])
        return lists
...
class simplex:
    ...
    def be():
        ...
        file_path = os.path.join(
            directory_path, 'icon.ico')
        rt.iconbitmap(file_path)
        ...
        # Title
        myLabel = Label(root, fg='#0D062E', bg='#7D7E7E', text="Simplex Method Calculator",
                        borderwidth=5, font=("Courier", 14, "bold"))
        ...
        myLabel2 = Label(root, fg='#0D062E', bg='#7D7E7E', text='Objective Function Z(x) = ', font=(
        ...
        myLabel3 = Label(root, fg='#0D062E', bg='#7D7E7E', text='Constraint Equations ', font=(
            'Helvetica', 10, 'bold', 'italic'))
        ...
        b1 = Button(root, text='Add Another Equation', borderwidth=2, bg='black',
                    fg='white', width=17, command=lambda: simplex.b_click(rt, root))
        b1.grid(row=4, column=1)

        b2 = Button(root, text='Maximize', borderwidth=2, bg='blue',
                    fg='white', width=12, command=lambda: simplex.calc_max(of, r))
        b2.grid(row=2, column=3)

        b3 = Button(root, text='Minimize', borderwidth=2, bg='red',
                    fg='white', width=12, command=lambda: simplex.calc_min(of, r))
        b3.grid(row=3, column=3)

        b4 = Button(root, text='Clear', borderwidth=2, width=12,
                    command=lambda: simplex.clear_entries(of))
    ...
    def calc_min(of, r):
        ...
        if inp == False:
            ...
            const = r2.getConstraintInp(
                obj, simplex.eq_num-3, simplex.plug, len(obj))
            ...
            if res != False:
                ...
                for i in range(len(res)-2):
                    if i > x_vars:
                        Label(frame, text='x'+str(count) +
                              ' = '+str(res[i])).pack()
                        count += 1
                Label(frame, text='Z(x) = ' +
                      str('{:f}'.format(res[len(res)-1]))).pack()
                Label(frame, text='').pack()
            else:
                messagebox.showerror(
                    'Error', 'The Problem Has An Infeasible Solution')
